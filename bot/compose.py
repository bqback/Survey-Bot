...
def get_question(update: Update, context: CallbackContext, returning = False) -> int:
	...
	update.message.reply_text(text = 'Введите текст вопроса №{}.').format(len(context.chat_data['current_survey']['questions'])+1)
...
def save_question(update: Update, context: CallbackContext) -> int:
	...
	update.message.reply_text(update.message.text)
	update.message.reply_text('Сохранить описание?', reply_markup = kb.SAVE_QUESTION_KB)
	return cc.GET_MULTIANS_STATE

def get_multi(update: Update, context: CallbackContext) -> int:
	query = update.callback_query
	query.answer()
	update.message.reply_text(text = 'У этого вопроса несколько вариантов ответа?', reply_markup = kb.YES_NO_KB)
...
def save_multi(update: Update, context: CallbackContext, multi: bool) -> int:
	...
	if multi:
		update.message.reply_text('В этом вопросе можно будет выбрать несколько вариантов ответа')
	else:
		update.message.reply_text('В этом вопросе можно будет выбрать только один вариант ответа')
	update.message.reply_text('Перейти к вводу ответов?', reply_markup = kb.SAVE_MULTI_KB)
...
def get_answer(update: Update, context: CallbackContext, returning = False) -> int:
	...
	update.message.reply_text(text = 'Введите текст ответа №{}.').format(len(context.chat_data['current_survey']['questions'][-1]['answers'])+1)
...
def save_answer(update: Update, context: CallbackContext) -> int:
	query = update.callback_query
	query.answer()
	update.message.reply_text('Выберите действие', reply_markup = kb.NEXT_ANSWER_KB)
	return cc.SAVE_ANSWER_STATE

def review(update: Update, context: CallbackContext, returning = False) -> int:
	query = update.callback_query
	query.answer()
	update.message.reply_text('Проверьте, правильно ли составлен опрос')
	if not returning:
		surv = context.chat_data['current_survey']
		questions_out = ""
		for question in surv['questions']:
			question_out.append(question['question'])
			if question['multi']:
	...
	update.message.reply_text(context.chat_data['survey_out'])
	update.message.reply_text('Выберите действие', reply_markup = kb.REVIEW_KB)
