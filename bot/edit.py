...
def title(update: Update, context: CallbackContext) -> int:
    ...
    query.edit_message_text(
            text = f"{text.CURRENT_TITLE}: {context.chat_data['current_survey']['title']}", 
...
def desc(update: Update, context: CallbackContext) -> int:
    ...
    query.answer()
    query.edit_message_text(
            text = f"{text.CURRENT_DESC}: {context.chat_data['current_survey']['desc']}", 
...
def questions(update: Update, context: CallbackContext) -> int:
    ...
    query.edit_message_text(
            text = f"{text.CURRENT_QUESTIONS}: {questions_out}", 
        )
