from .queuebot import MQBot, QueueUpdater
...
def build(token: str, use_context: bool, persistence: PicklePersistence, defaults: Dict) -> Updater:
	...
	bot = MQBot(token = token, request = request, mqueue = queue)
	defaults = Defaults(defaults)
	updater = QueueUpdater(bot = bot, use_context = True, persistence = persistence, defaults = defaults)
	return updater
